      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-10 max-w-5xl mx-auto">
        <div className="bg-white p-6 rounded-xl shadow-lg border">
          ...
          <ReactSortable
            ...
            animation={150}
            tag="ul"
            // swap          // 🔄 Enable swapping behavior
            // swapThreshold={0.7}         // ✅ Apply swap threshold
            // fallbackTolerance={4}      // ✅ Set mouse/touch drag tolerance
            // touchStartThreshold={2}
            className="space-y-3"
          >
            {Items.map((item) => (
              <li key={item.id}>
                <div className="bg-gray-100 hover:bg-gray-200 text-gray-800 py-3 px-5 rounded-lg transition-all duration-150 shadow-sm cursor-move">
                  {item.name}
                </div>
              </li>
            ))}
